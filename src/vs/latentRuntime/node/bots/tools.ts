...
export const builtinTools: Record<string, IRuntimeTool> = {
	...
	memory: {
		definition: { name: 'memory', description: 'Read or manage bounded persistent MEMORY.md notes or USER.md preferences. Add, replace or remove whole entries using a unique oldText substring. A batch operations array consolidates atomically within the final budget. Destructive changes are staged for user review. Save durable facts, never secrets or transient task state.', parameters: { type: 'object', properties: { target: { type: 'string', enum: ['memory', 'user'] }, action: { type: 'string', enum: ['read', 'add', 'replace', 'remove'] }, content: { type: 'string' }, oldText: { type: 'string' }, operations: { type: 'array', items: { type: 'object', properties: { action: { type: 'string', enum: ['add', 'replace', 'remove'] }, content: { type: 'string' }, oldText: { type: 'string' } }, required: ['action'] } } }, required: ['action'] } },
		run: async (args, context) => args.action === 'read' ? context.memoryRead?.() ?? 'Memory reading is unavailable.' : context.memoryManage ? context.memoryManage(args as unknown as IMemoryWriteOp) : 'Memory management is unavailable.',
	},
	memory_write: {
