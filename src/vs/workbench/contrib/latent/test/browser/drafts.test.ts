...
import { findReferences, rewriteReferencesForModel, validateDraftReferences } from '../../common/drafts.js';
...
suite('Latent drafts', () => {
	...
	test('finds #n references but not words or hashes', () => {
		assert.deepStrictEqual(findReferences('see #1 and #12, not a#3 or ##4 or #x').map(r => r.number), [1, 12]);
	});
	...
	test('valid references are rewritten for the model only (P1-AS-013)', () => {
		...
		assert.strictEqual(rewriteReferencesForModel('compare #1 and #3', attachments), 'compare [#1: a.md] and [#3: selection]');
		assert.deepStrictEqual(validateDraftReferences('compare #1 and #3', attachments, 4), []);
	});
