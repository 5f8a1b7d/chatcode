(function () {
	...
	function render() {
		...
		list.append(element('div', 'list-title', category === 'token-plan' ? 'Plans' : 'Catalog'));
		if (category === 'token-plan') {
	...
	function renderProvider(detail, provider) {
		...
		const customModel = ['llm', 'image', 'video', 'tts', 'asr'].includes(provider.service) ? field(card, 'Add Model ID', '', 'text', 'Optional custom model') : undefined;
		const extraInputs = [];
	...
	function renderCustomForm(detail) {
		...
		const protocol = category === 'llm' ? select(card, 'Protocol', [{ id: 'openai', name: 'OpenAI Compatible' }, { id: 'anthropic', name: 'Anthropic' }, { id: 'google', name: 'Google Gemini' }, { id: 'azure', name: 'Azure OpenAI' }], 'openai') : undefined;
		const url = field(card, 'Base URL', '', 'url', 'https://…');
		const model = ['llm', 'image', 'video', 'tts', 'asr'].includes(category) ? field(card, 'Model ID', '', 'text', 'Optional') : undefined;
	...
	window.addEventListener('message', event => {
		...
		if (message.type === 'init') { catalog = message.catalog; saved = message.state; render(); showStatus('Saved'); }
